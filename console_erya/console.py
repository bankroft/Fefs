...
class Console:
    ...
    def init(self):
        self.driver = webdriver.Chrome(
            cc.chrome_drive_path, chrome_options=self.options)
    ...
    def login(self, student_num, password, code):
        try:
            self.driver.find_element(
                cc.login_code['type'], cc.login_code['string']).clear()
        except common.exceptions.NoSuchElementException:
            self.driver.switch_to.frame(
                self.driver.find_elements_by_name('iframe')[0])
            self.driver.find_element(
                cc.login_code['type'], cc.login_code['string']).clear()
        self.driver.find_element(
            cc.login_code['type'], cc.login_code['string']).send_keys(code)
        self.driver.find_element(
            cc.login_username['type'], cc.login_username['string']).clear()
        self.driver.find_element(
            cc.login_username['type'], cc.login_username['string']).send_keys(student_num)
        self.driver.find_element(
            cc.login_password['type'], cc.login_password['string']).clear()
        self.driver.find_element(
            cc.login_password['type'], cc.login_password['string']).send_keys(password)
        self.driver.find_element(
            cc.login_button['type'], cc.login_button['string']).click()
        self.driver.switch_to.default_content()
        try:
            name = self.driver.find_element(
                cc.login_ver['type'], cc.login_ver['string']).text
        except common.exceptions.NoSuchElementException:
            try:
                result_text = self.driver.find_element(
                    cc.login_result['type'], cc.login_result['string']).text
            except common.exceptions.NoSuchElementException:
                return 'False', False
    ...
    def search_school(self, school):
        ...
        """
        self.driver.find_element(
            cc.select_school_button['type'], cc.select_school_button['string']).click()  # 点击选择院校按钮
        # 是否已经点击过院校选择按钮进入院校选择界面
        # self.driver.find_element('css', '#dialog2 > div > div.zw_result > p > a').click()
        self.driver.find_element(
            cc.select_school_search['type'], cc.select_school_search['string']).clear()  # 清除搜索框
        self.driver.find_element(
            cc.select_school_search['type'], cc.select_school_search['string']).send_keys(school)  # 向搜索框填入院校
        self.driver.find_element(
            cc.select_school_search_button['type'], cc.select_school_search_button['string']).click()  # 院校搜索
        self.__select_school_result = self.driver.find_elements(
            cc.select_school_result['type'], cc.select_school_result['string'])
    ...
    def get_login_ver_code(self, refresh=False, display=False):
        ...
        if refresh:
            self.driver.find_element(
                cc.refresh_code['type'], cc.refresh_code['string']).click()
        ...
        a = self.driver.find_element(
            cc.code['type'], cc.code['string'])  # 定位验证码
        l = a.location['x'] + 1
        t = a.location['y'] + 1
    ...
    def get_course(self):
        ...
        try:
            self.driver.find_element(
                cc.lesson_index['type'], cc.lesson_index['string']).click()
        except common.exceptions.NoSuchElementException:
            pass
        except:
            pass
    ...
    def browse_watch(self, course_id):
        ...
        self.driver.find_elements(
            cc.first_lesson['type'], cc.first_lesson['string'])[0].click()
...
class Exam:
    ...
    def start(self):
        ...
        typ = [(x, y.text.strip())
               for x, y in enumerate(a.find_elements_by_tag_name('h3'))]
        for index, value in typ:
            z = 0
            a = self.driver.find_element_by_class_name('leftCardChild')
            le = len(a.find_elements_by_tag_name('div')[
                     index].find_elements_by_tag_name('a'))
            for x in range(0, le):
                z += 1
                a = self.driver.find_element_by_class_name('leftCardChild')
                a.find_elements_by_tag_name(
                    'div')[index].find_elements_by_tag_name('a')[x].click()
                time.sleep(3)
                ...
                right_answer = query_http_server(
                    op='query', test_type=value, title=title)
                if value == '判断题':
                    if right_answer[0]:
                        if right_answer[2]:
                            print_info(['查询到', title, '正确'], 'info', True)
                            self.driver.find_elements_by_xpath(
                                '//*[@id="submitTest"]//ul[@class="Cy_ulBottom clearfix"]//li')[0].click()
                        else:
                            print_info(['查询到', title, '错误'], 'info', True)
                            self.driver.find_elements_by_xpath(
                                '//*[@id="submitTest"]//ul[@class="Cy_ulBottom clearfix"]//li')[1].click()
                    else:
                    ...
                    if not tag:
                        if right_answer[3] in ['token', 'open']:
                            ...
                            right_answer = query_http_server(
                                op='query', defalut=False, test_type=value, title=title)
                            tag = 0
                            if right_answer[0]:
                                print_info(['查询到', title, ' '.join(
                                    right_answer[2])], 'info', True)
                                ...
                                print_info(
                                    ['查询失败', title, '随机选择一项'], 'notice', True)
                        self.driver.find_elements_by_xpath(
                            '//*[@id="submitTest"]//ul[@class="Cy_ulTop w-top"]/li')[0].click()
                else:
