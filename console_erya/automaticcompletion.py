...
class AutomaticCompletion:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                les = self.driver.find_element(
                    ac.not_completed_lesson['type'], ac.not_completed_lesson['string'])
                ActionChains(self.driver).click(les).perform()
                time.sleep(5)
                now_lesson = self.driver.find_element(
                    ac.lesson_name['type'], ac.lesson_name['string']).text
                if last_lesson == now_lesson:
                    if retry == 0:
                        print_info(['出错', '无法检测视频(【{0}】)状态,请关闭程序手动观看该节课程'.format(
                            last_lesson), '重试'], 'verbose', True, all_output=True)
                        return False, False
                    else:
                        print_info(['出错', '视频(【{0}】)播放错误'.format(
                            last_lesson), '重试'], 'warning', True, all_output=True)
                        retry -= 1
                    if refresh_tag:
                        refresh_tag = 0
                    else:
                        print_info(['刷新页面', '视频(【{0}】)'.format(
                            last_lesson), '刷新'], 'warning', True, all_output=True)
                        self.driver.refresh()
                        refresh_tag = 1
                        retry += 1
                        continue
                    ...
                    retry = 3
                last_lesson = self.driver.find_element(
                    ac.lesson_name['type'], ac.lesson_name['string']).text
                ...
                self.driver.get_screenshot_as_file(os.path.join(
                    folder_temp_path, ''.join(choices(ascii_letters, k=8))+'.png'))
                if not ans and not updb:
                    if now_lesson.strip() == '阅读':
                        print_info(['出错', '阅读页面', '请手动完成'], 'error',
                                   timz=True, all_output=True)
                        return False, False
                    else:
                        print_info(['警报', '该章节可能无章节测验', '跳过'],
                                   'warning', True, all_output=True)
                        return False, False
                # time.sleep(10)
            except common.exceptions.NoSuchElementException:
                break
        print_info(
            ['课程', self.course_name if self.course_name else '没记录课程名字', '完成'], 'info', True)
        return True
    ...
    def switch_to_video_frame(self):
        ...
        for x in ac.learn_page_video_iframe:
            ...
            self.driver.switch_to.frame(
                self.driver.find_elements_by_tag_name(x['name'])[x['index']])

    def play_status(self):
        # self.switch_to_video_frame()
        self.__screenshot_video(os.path.join(
            folder_temp_path, str(0) + '.png'))
        time.sleep(5)
        # 秒
        progress = -1
        self.__screenshot_video(os.path.join(
            folder_temp_path, str(1) + '.png'))
        if open(os.path.join(folder_temp_path, str(0) + '.png'), 'rb').read() != open(os.path.join(folder_temp_path, str(1) + '.png'), 'rb').read():
            self.switch_to_video_frame()
            try:
                a = int(float(self.driver.find_element(
                    ac.video_object['type'], ac.video_object['string']).get_attribute('duration')))
                b = int(float(self.driver.find_element(
                    ac.video_object['type'], ac.video_object['string']).get_attribute('currentTime')))
                progress = b, a
            except common.exceptions.NoSuchElementException:
                pass
    ...
    def answer_video(self, qt=None):
        ...
        mid = self.driver.find_element_by_tag_name(
            'iframe').get_attribute('mid')
        dc = int(time.time()*1000)
        try:
            res = requests.get(url=ac.video_answer_url.format(
                mid, dc), headers=headers).json()
        except requests.exceptions.ConnectionError:
            print_info(['视频答题', '获取失败', '返回'], 'warning', True)
            return False
        print_info(['视频答题', res[0]['datas'][0]
                    ['description'], '正在处理'], 'info', True)
        self.switch_to_video_frame()
        for k, d in enumerate(res[0]['datas'][0]['options']):
            if d['isRight']:
                self.driver.find_element_by_xpath(
                    '//div[@class="ans-videoquiz-title"]/../ul/li[' + str(k+1) + ']/label').click()
        self.driver.find_element_by_xpath(
            '//div[@class="ans-videoquiz-title"]/../div[2]').click()
        print_info(['视频答题', res[0]['datas'][0]
                    ['description'], '完成'], 'info', True)
    ...
    def video_complete_status(self):
        ...
        try:
            self.driver.find_element(
                ac.video_complete_status['type'], ac.video_complete_status['string'])
            return True
        except common.exceptions.NoSuchElementException:
            pass
        try:
            self.driver.find_element(
                ac.video_not_complete_status['type'], ac.video_not_complete_status['string'])
            return False
        except common.exceptions.NoSuchElementException:
            return True
    ...
    def __watch(self):
        ...
        try:
            self.driver.find_element(
                ac.learn_page_video_button['type'], ac.learn_page_video_button['string']).click()
            # time.sleep(2)
        except common.exceptions.NoSuchElementException:
            return True
        # ct: 改变线路，a: 回答,s: 点击开始
            ...
            if a[0]:
                ...
                try:
                    ...
                    text = self.driver.find_element_by_xpath(
                        '//*[@aria-label="弹窗"]/div[2]/div').text.strip()
                    # 是否正确切换线路
                    tmp = False
                    if '视频因格式不支持或者服务器或网络的问题无法加载' in text:
                        for x in self.driver.find_elements_by_xpath('//*[@aria-label="弹窗"]/div[2]/ul/li/label'):
                            if x.text.strip() == ac.internet_line:
                                print_info(
                                    ['切换线路', ac.internet_line, '完成'], 'info', True)
                                x.click()
                                tmp = True
                                time.sleep(3)
                        ...
                        if not tmp:
                            print_info(
                                ['切换线路', ac.internet_line, '失败'], 'notice', True)
    ...
    def __answer(self):
        ...
        try:
            self.driver.find_element(
                ac.learn_page_test_button['type'], ac.learn_page_test_button['string']).click()
        except common.exceptions.NoSuchElementException:
        ...
        while True:
            ...
            try:
                # 页面是否加载完成
                self.driver.find_element(
                    ac.test_load_complete_tag['type'], ac.test_load_complete_tag['string'])
            except common.exceptions.NoSuchElementException:
                print_info(['Error', 'Unknown error',
                            'Continue'], 'warning', True)
                continue
            break
        self.driver.switch_to.default_content()
        ...
        try:
            # xpath可能有问题
            self.driver.find_element(
                ac.test_complete_stataus['type'], ac.test_complete_stataus['string'])
            return True
        except common.exceptions.NoSuchElementException:
            pass
        self.driver.switch_to.default_content()
        ...
        while True:
            try:
                ...
                break
            title = tmp.find_element_by_class_name('clearfix').text.strip(
                '1234567890').replace('\n', '').strip()
            test_type = title[1:4]
            if test_type not in ['判断题', '单选题', '多选题']:
                print_info(['查询', test_type + '暂不支持', '跳过'], 'notice', True)
                continue
            right_answer = query_http_server(
                op='query', test_type=test_type, title=title[5:])
            if test_type == '判断题':
                if right_answer[0]:
                    if right_answer[2]:
                        print_info(['查询到', title, '正确'], 'info', True)
                        tmp.find_elements_by_tag_name(
                            'ul')[0].find_elements_by_tag_name('label')[0].click()
                    else:
                        print_info(['查询到', title, '错误'], 'info', True)
                        tmp.find_elements_by_tag_name(
                            'ul')[0].find_elements_by_tag_name('label')[1].click()
                else:
                ...
                tag = 0
                if right_answer[0]:
                    print_info(['查询到', title, ' '.join(
                        right_answer[2])], 'info', True)
                ...
                if not tag:
                    if right_answer[3] in ['token', 'open']:
                        ...
                        right_answer = query_http_server(
                            op='query', defalut=False, test_type=test_type, title=title[5:])
                        tag = 0
                        if right_answer[0]:
                            print_info(['查询到', title, ' '.join(
                                right_answer[2])], 'info', True)
                    ...
                    # 未搜索到该题目答案，随机选择一项
                    tmp.find_elements_by_tag_name('ul')[0].find_elements_by_tag_name('li')[randint(0, len(
                        tmp.find_elements_by_tag_name('ul')[0].find_elements_by_tag_name('li'))-1)].click()
        # 提交部分
        print_info(['提交章节测试', '如果有未查到的单/多选题会等待{0}分钟提交，默认10s后提交'.format(
            ac.noanswer_sleep.__str__()), '等待'], 'info', timz=True, all_output=True)
        ...
        for x in ac.test_submit_iframe:
            self.driver.switch_to.frame(
                self.driver.find_elements_by_tag_name(x['name'])[x['index']])
        self.driver.find_element(
            ac.submit_test['type'], ac.submit_test['string']).click()
        time.sleep(2)
        while True:
            if self.driver.find_element(ac.submit_test_confirm['type'], ac.submit_test_confirm['string']).text != '确定':
                tmp = self.driver.find_element(
                    ac.submit_test_validate['type'], ac.submit_test_validate['string'])
                if tmp.is_displayed():
                    os.system(self.__screenshot_sbt_validate())
                    code = input('验证码:').strip()
                    self.driver.find_element(
                        ac.submit_test_code_input['type'], ac.submit_test_code_input['string']).clear()
                    self.driver.find_element(
                        ac.submit_test_code_input['type'], ac.submit_test_code_input['string']).send_keys(code)
                    self.driver.find_element(
                        ac.submit_test_code_button['type'], ac.submit_test_code_button['string']).click()
            else:
                break
        self.driver.find_element(
            ac.submit_test_confirm['type'], ac.submit_test_confirm['string']).click()
    ...
    def __update_db(self):
        ...
        for x in self.driver.find_elements_by_class_name('TiMu'):
            title = x.find_elements_by_tag_name(
                'div')[1].text.strip().replace('\n', '')
            test_type = title[1:4]
            if test_type not in ['判断题', '单选题', '多选题']:
            ...
            try:
                # 选择题正确icon
                right_or_wrong = x.find_element_by_tag_name('form').find_element_by_tag_name(
                    'div').find_element_by_tag_name('i').get_attribute('class')
            except (common.exceptions.NoSuchElementException, IndexError):
                # 判断题正确icon
                right_or_wrong = x.find_elements_by_tag_name(
                    'div')[4].find_elements_by_tag_name('i')[1].get_attribute('class')
            if right_or_wrong == 'fr dui':
                ...
                if test_type == '判断题':
                    if query_http_server(op='update', title=title[5:], test_type=test_type, answer=my_answer.strip()):
                        print_info(['更新题库', title + '\t答案: ' +
                                    my_answer, '成功'], 'info', True)
                    else:
                        print_info(['更新题库', title + '\t答案: ' +
                                    my_answer, '失败'], 'warning', True)
                else:
                    ma = re.findall('[{0}]'.format(self.__select), my_answer)
                    r = []
                    for y in ma:
                        r.append(x.find_elements_by_tag_name('li')[self.__select.index(
                            y)].text.strip(self.__select).strip('、').strip())
                    answer = '&'.join(r)
    ...
    def __screenshot_sbt_validate(self, filename=os.path.join(folder_temp_path, 'sbtcode.png')):
        ...
        self.driver.switch_to.default_content()
        tmp = self.driver.find_element(
            ac.submit_test_imgcode['type'], ac.submit_test_imgcode['string'])
        i = Image.open('tmp.png')
        i.crop(
            (
